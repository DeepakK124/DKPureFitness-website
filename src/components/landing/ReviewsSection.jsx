...
function getTrackWidth() {
  if (typeof window === 'undefined') return REVIEWS.length * (320 + 16);
  const w = window.innerWidth;
  const cardW = w < 640 ? 224 : w < 768 ? 256 : 320;
  const gap = w < 640 ? 16 : 24;
  return REVIEWS.length * (cardW + gap);
}
...
export default function ReviewsSection() {
  ...
  // Lazy initializer avoids wrong width on first render (especially on mobile)
  const [trackWidth, setTrackWidth] = useState(getTrackWidth);

  useEffect(() => {
    const handleResize = () => setTrackWidth(getTrackWidth());
    ...
    <section id="reviews" className="relative py-16 sm:py-24 md:py-32 overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden mt-8 sm:mt-12">
        ...
        <motion.div
          ...
          animate={shouldReduceMotion ? {} : { x: [0, -trackWidth] }}
          transition={
            shouldReduceMotion
              ? {}
              : { repeat: Infinity, duration: REVIEWS.length * 5, ease: 'linear' }
          }
